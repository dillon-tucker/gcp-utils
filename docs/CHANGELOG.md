...
*   Removed invalid `retry.DEFAULT` references from `src/gcp_utilities/controllers/storage.py` (lines 251, 313, 373, 418, 463) - Google Cloud Storage client has built-in retry logic, so explicit retry parameter is not needed.
...
*   Updated all Pydantic models in `src/gcp_utilities/models/` to replace deprecated `json_encoders` with the modern `@field_serializer` decorator for custom `datetime` and `timedelta` serialization. This change eliminates deprecation warnings and aligns with Pydantic v2 best practices. Affected files include:
    *   `src/gcp_utilities/models/storage.py`
    *   `src/gcp_utilities/models/firestore.py`
    *   `src/gcp_utilities/models/cloud_run.py`
    *   `src/gcp_utilities/models/workflows.py`
    *   `src/gcp_utilities/models/tasks.py`
    *   `src/gcp_utilities/models/firebase_hosting.py`
    *   `src/gcp_utilities/models/artifact_registry.py`
...
*   Fixed type annotation issues in `src/gcp_utilities/config/settings.py`:
